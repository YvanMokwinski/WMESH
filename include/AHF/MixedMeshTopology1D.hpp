...
namespace AHF
{
  using MixedMeshTopology1D = MeshTopology<DimensionType::Edge>;
};

//!
//! Implementation of the AHF 1D topology.
//!
template<> struct CRTP_MeshTopology_traits<AHF::MixedMeshTopology1D>
{
  //
...
namespace AHF
{
  ...
  template <>
  inline unsigned int MixedMeshTopology1D::GetEntityToEntities<DimensionType::Volume,DimensionType::Node>
  ...
  template <>
  inline unsigned int MixedMeshTopology1D::GetEntityToNodes<DimensionType::Edge>(const entity_t<DimensionType::Volume>& 	cell_,
									    const unsigned int 				localEntityIndex_,
									    entity_t<DimensionType::Node> 		entityToNodes_[]) const noexcept
  {
  ...
  template <>
  inline unsigned int MixedMeshTopology1D::GetEntityToNodes<DimensionType::Face>(const entity_t<DimensionType::Volume>& 	cell_,
									    const unsigned int 				localEntityIndex_,
									    entity_t<DimensionType::Node> 		entityToNodes_[]) const noexcept
  {
    const auto cellKind 		= cell_.GetKind();
  ...
  template <> inline int_t MixedMeshTopology1D::GetNumEntities<DimensionType::Node>() const noexcept
  {
    return this->GetNumNodes();
  };

  template <> inline int_t MixedMeshTopology1D::GetNumEntities<DimensionType::Volume>() const noexcept
  {
  ...
  template <>
  inline int_t MixedMeshTopology1D::GetNumEntities<DimensionType::Volume>(const typename VolumeType::enum_t cellType_) const noexcept
  {
    return this->m_numCells[cellType_];
  };
  
  template <>
  inline RangeMeshEntities<DimensionType::Node> MixedMeshTopology1D::GetEntities<DimensionType::Node>(const NodeType::enum_t entityKind_) const noexcept
  {
    return RangeMeshEntities<DimensionType::Node>(MeshNode(0),MeshNode(m_numNodes));
  };
  
  template <>
  inline RangeMeshEntities<DimensionType::Volume> MixedMeshTopology1D::GetEntities<DimensionType::Volume>(const VolumeType::enum_t entityKind_) const noexcept
  {
    return RangeMeshEntities<DimensionType::Volume>(MeshCell(0,entityKind_),
